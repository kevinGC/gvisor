...
func (NATPreOriginalDstUnchanged) ContainerAction(ip net.IP) error {
	...
	if _, ok := err.(originalDstError); err == nil || !ok {
		return fmt.Errorf("Expected SO_ORIGINAL_DST to fail, but instead encountered error: %v", err)
	}
	return nil
}
...
func listenForRedirectedConn(originalDst net.IP) error {
	...
	_, _, errno := syscall.Syscall6(
		...
		0)
	if errno != 0 {
		return originalDstError(fmt.Sprintf("errno (%d) when calling getsockopt(SOL_IP, SO_ORIGINAL_DST): %v", int(errno), errno.Error()))
	}
