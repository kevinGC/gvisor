...
// Represents a match for a rule. A single rule can have many of these.
struct ipt_entry_match {
  ...
  // This runs past the end of the struct.
  unsigned char data[0];
};

// Represents a target of a rule. A single rule can have only one of these.
...
// A builting target: ACCEPT, DROP, JUMP, QUEUE, or RETURN.
struct ipt_standard_target {
  struct ipt_entry_target target;
  int verdict;
};

// An error target, which is used to mark the beginning of a user-defined chain.
