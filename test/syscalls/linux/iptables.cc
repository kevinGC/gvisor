...
// TODO: test that we can only use iptables on raw sockets.
...
constexpr size_t kEmptyStandardEntrySize = sizeof(struct ipt_entry) + sizeof(struct ipt_standard_target);
constexpr size_t kEmptyErrorEntrySize = sizeof(struct ipt_entry) + sizeof(struct ipt_error_target);

TEST(IPTablesBasic, CreateSocket) {
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_NET_RAW)));

  int sock;
  ASSERT_THAT(sock = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP), SyscallSucceeds());
...
TEST(IPTablesBasic, FailSockoptNonRaw) {
  ...
  EXPECT_THAT(getsockopt(sock, IPPROTO_IP, SO_GET_INFO, &info, &info_size), SyscallFailsWithErrno(ENOPROTOOPT));
...
// test that gVisor has the same initial state that a
// newly-booted Linux machine would have.
TEST_F(IPTablesTest, InitialState) {
  ...
  snprintf(info.name, XT_TABLE_MAXNAMELEN, "%s", kNatTablename);
  socklen_t info_size = sizeof(info);
  ASSERT_THAT(getsockopt(s_, IPPROTO_IP, SO_GET_INFO, &info, &info_size), SyscallSucceeds());
  ...
  // TODO: Also test GetEntries.
  socklen_t entries_size = sizeof(struct ipt_get_entries) + info.size;
  struct ipt_get_entries* entries = static_cast<struct ipt_get_entries*>(malloc(entries_size));
  snprintf(entries->name, XT_TABLE_MAXNAMELEN, "%s", kNatTablename);
  entries->size = info.size;
  ASSERT_THAT(getsockopt(s_, IPPROTO_IP, SO_GET_ENTRIES, entries, &entries_size), SyscallSucceeds());
  ...
  while (entry_offset < entries->size) {
    struct ipt_entry *entry = reinterpret_cast<struct ipt_entry*>(reinterpret_cast<char*>(entries->entrytable) + entry_offset);

    // ip should be zeroes.
    struct ipt_ip zeroed = {};
    EXPECT_EQ(memcmp(static_cast<void*>(&zeroed), static_cast<void*>(&entry->ip), sizeof(zeroed)), 0);
    ...
    if (entry_offset < kEmptyStandardEntrySize * 4) {
      ...
      struct ipt_standard_target* target = reinterpret_cast<struct ipt_standard_target*>(entry->elems);
      ...
      struct ipt_error_target* target = reinterpret_cast<struct ipt_error_target*>(entry->elems);
