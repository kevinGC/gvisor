...
func (f *fakeNetworkEndpoint) HandlePacket(r *stack.Route, pb *buffer.PacketBuffer) {
	// Increment the received packet count in the protocol descriptor.
	f.proto.packetCount[int(f.id.LocalAddress[0])%len(f.proto.packetCount)]++

	// Consume the network header.
	b := pb.Data.First()
	pb.Data.TrimFront(fakeNetHeaderLen)
	...
	if b[2] == uint8(fakeControlProtocol) {
		...
		pb.Data.TrimFront(fakeNetHeaderLen)
		f.dispatcher.DeliverTransportControlPacket(tcpip.Address(nb[1:2]), tcpip.Address(nb[0:1]), fakeNetNumber, tcpip.TransportProtocolNumber(nb[2]), stack.ControlPortUnreachable, 0, pb)
		return
	}

	// Dispatch the packet to the transport protocol.
	f.dispatcher.DeliverTransportPacket(r, tcpip.TransportProtocolNumber(b[2]), pb)
}
...
func TestNetworkReceive(t *testing.T) {
	...
	ep.InjectInbound(fakeNetNumber, &pb)
	...
	ep.InjectInbound(fakeNetNumber-1, &pb)
	if fakeNet.packetCount[1] != 1 {
		t.Errorf("packetCount[1] = %d, want %d", fakeNet.packetCount[1], 1)
	}
	if fakeNet.packetCount[2] != 1 {
...
func TestNICStats(t *testing.T) {
	...
	ep1.InjectInbound(fakeNetNumber, &pb)
